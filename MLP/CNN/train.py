...
from nn.model import MultiChannelCNN, RestNN
...
def main(args):
    ...
    df_total, df_main = get_time_series(df)
    real = df_main[-params['tst_size']:]
    
    trn_total = TimeSeriesDataset(df_total[:-params['tst_size']], 
                                 params['input_size'], params['pred_size'])
    trn_main = TimeSeriesDataset(df_main[:-params['tst_size']], 
                                 params['input_size'], params['pred_size'])
    tst_total = TimeSeriesDataset(df_total[-params['tst_size']-params['input_size']:], 
                                 params['input_size'], params['pred_size'])
    tst_main = TimeSeriesDataset(df_main[-params['tst_size']-params['input_size']:], 
                                 params['input_size'], params['pred_size'])
    

    trn_total_dl = torch.utils.data.DataLoader(trn_total, batch_size=dl_params.get("batch_size"), 
                                               shuffle=dl_params.get("shuffle"))
    tst_total_dl = torch.utils.data.DataLoader(tst_total, batch_size=params['tst_size'], shuffle=False)
    ...
    if args.get("multi"):
        nets['multi_channel'] = (MultiChannelCNN(params['input_size'],params['pred_size'],params['hidden_dim'],3).to(device))
    if args.get("resnet"):
