...
class PatchTSDataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, i):
    look_back = self.data[i:(i+self.window_size)] #[1,...,48]
    look_back = np.concatenate([look_back]+[look_back[-int(self.patch_length/4):]]*2) #[0,...,48]+[4]*2
    x = np.array([look_back[i*int(self.patch_length/2):(i+2)*int(self.patch_length/2)] for i in range(self.n_patches)])
    ...
    y = self.data[(i+self.window_size):(i+self.window_size+self.prediction_length)]
    return x, y